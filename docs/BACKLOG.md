...
- [x] `M3-06` Credential prompt as a docked panel (airspace-safe, not a modal) — **M** — docked, not overlaid, and the difference is the point. `SessionSurface` already knew how to swap a session for WPF content, because WPF drawn over a hosted window is invisible (M4-03). A prompt wants neither: somebody answering one is usually reading the logon screen it is asking about. So the surface grew a second row that takes space rather than covering — showing the panel makes the session shorter, and the session stays visible. The dock is independent of the swap, so a panel works over an overlay too, which is what a tab that failed on a refused password shows.
  ...
  The per-session cache and the ask-before-connecting flow landed with `M3-05`, which also gave the panel its second button. Still not here: any way to make a profile from the panel, which is `M3-10`
